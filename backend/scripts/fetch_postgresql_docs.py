...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fetch PostgreSQL docs sources (SGML) into a local domain data folder."
    )
    parser.add_argument("--out", default="domains/postgresql_docs/data", help="Output directory for docs files")
    ...
    with tempfile.TemporaryDirectory(prefix="postgres-docs-") as td:
        clone_dir = Path(td) / "postgres"
        try:
            if args.commit:
        ...
        sgml_files = sorted(docs_root.rglob("*.sgml"))
        if not sgml_files:
            print(f"ERROR: no SGML files found under {docs_root}", file=sys.stderr)
            return 2

        for src in sgml_files:
        ...
        print(f"Copied {len(sgml_files)} files to {out_dir}")
